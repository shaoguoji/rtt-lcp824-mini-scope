...
{
    rt_memset(OLED_GRAM, 0x00, 8*128);
}

//清显存函数
void OLED_BUF_Copy(const void *src)
{
    rt_memcpy(OLED_GRAM, src, 8*128);
}

void OLED_Left_Shift_Pixel(void)
{
    rt_memmove(OLED_GRAM, &OLED_GRAM[1][0], 127*8);
    rt_memset(&OLED_GRAM[127][0], 0x00, 8);
}
...
void OLED_ScrollDisplay(u8 num,u8 space,u8 mode)
{
    ...
    while(1)
    {
        ...
        //  t++;
            OLED_ShowString(8,16,"www.eetree.cn",16,1);
        }
        if(t==num)
            {
